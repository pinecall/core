...
export default defineTool({
    ...
    manual: "Pick a row from `list_voices` and pass its `voice` verbatim. Audio leaves the USER's speakers: say so first. ElevenLabs/Cartesia return a fixed clip, so `text` is ignored; `played: false` → give the `previewUrl`.",
    async handler(
