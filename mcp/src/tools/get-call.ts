...
export default defineTool({
    ...
    manual: [
        "**`get_call`** — this is the debugger. After ANY chat or phone call, READ THE LOG",
        "instead of guessing why the agent behaved as it did: the transcript, the tool calls",
        "with their real arguments and results, and the turn latencies are all in here.",
        "",
        "`{ call_id, after?, agent?, limit? }` → `{ call, agent, phase, live, messages: [{ seq, role, text }],",
        "toolCalls: [{ name, args, result }], turns: [{ turn, latency }], summary?, gaps?, nextAfter, truncated }`.",
        "",
        "Ephemeral entries (partial transcripts, per-word timing) are collapsed — a message is",
        "the final text, with the `seq` of the entry that produced it.",
        "`truncated: true` means the call is longer than one page: call again with",
        "`after = nextAfter` to get the rest. `gaps` means the server could not replay from",
        "your cursor (the hot buffer moved on) — the entries in that range are gone, not lost by you.",
        "",
        "Debug order: `list_calls(agent)` → the newest `call` → `get_call(call)`.",
    ].join("\n"),
    async handler(
